...
public class OniGroupControl : MonoBehaviour {
	...
	{
		// 显示碰撞器（用于调试）
		this.gameObject.renderer.enabled = SceneControl.IS_DRAW_ONI_GROUP_COLLISION;
	...
	public void OnAttackedFromPlayer()
	{
		...
		foreach(OniControl oni in this.onis) {
			...
			if(!SceneControl.IS_ONI_BLOWOUT_CAMERA_LOCAL) {
				...
				// 要加上玩家的速度
				blowout += this.player.rigidbody.velocity;
			}
		...
		if(this.onis.Length > 0)
		{
			...
			if( yarareSE != null )
			{
				...
				this.onis[0].audio.clip = yarareSE[index];
				this.onis[0].audio.Play();
			}
		}

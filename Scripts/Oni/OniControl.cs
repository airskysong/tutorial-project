...
public class OniControl : MonoBehaviour {
	...
	void	Update()
	{
		...
		if(this.next_step != STEP.NONE) {
			...
			switch(this.next_step) {
				...
				case STEP.DEFEATED:
				{
					this.rigidbody.velocity = this.blowout_vector;

					// 旋转的角速度
					this.rigidbody.angularVelocity = this.blowout_angular_velocity;
					...
					// 播放被攻击的动作
					this.transform.GetChild(0).animation.Play("oni_yarare");
		...
		switch(this.step) {
			...
			case STEP.DEFEATED:
			{
				...
				if(this.transform.parent != null) {
					...
					this.rigidbody.velocity += -3.0f*Vector3.right*Time.deltaTime;
				}
			}
			break;
	...
	{
		this.transform.GetChild(0).animation["oni_run1"].speed = speed;
		this.transform.GetChild(0).animation["oni_run2"].speed = speed;
	}

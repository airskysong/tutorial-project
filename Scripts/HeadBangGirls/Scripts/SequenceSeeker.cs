...
//private variables
	int		m_nextIndex = 0;				//查找位置（＝从现在时刻来看，下一个元素的索引）
	float	m_currentBeatCount = 0;			//现在时刻
	bool	m_isJustPassElement = false;	//只有到达了查找位置的那一帧才会为true

	List<ElementType> m_sequence;			//查找的序列数据
}
